...
- `packages/web/server/lib/ui-plugins/routes.js`: authenticated read-only catalog for declarative UI-extension manifests. The catalog exposes data contracts only—no renderer bundles, filesystem paths, credentials, or executable plugin code. Desktop inherits it through the shared Web server. This UI-only discovery route is deliberately not published through AgentGateway because it provides no agent capability or mutation surface.
- `packages/web/server/lib/company-office/runtime.js`: bounded read-only company projection. It reads the configured roster and live OpenCode sessions, then joins normalized work items from the configured tracker connector; credentials and raw upstream responses never enter the browser contract. See `packages/web/server/lib/company-office/DOCUMENTATION.md` for the installation-neutral contract and webhook boundary.
- `packages/web/server/lib/company-office/routes.js`: authenticated `GET /api/company-office/snapshot` route. It is enabled with `OPENCHAMBER_COMPANY_OFFICE_CONFIG` and returns explicit unavailable errors instead of an authoritative empty snapshot. `packages/web/company-office.config.example.json` documents the deployment-time shape without containing the Jira token itself.
