...
- Phones and tablets share one navigation model: a sessions drawer/sidebar on the left, app-level pages (including configured workspace views such as Company Office) in its footer, the workspace drawer (Changes / Files / Terminal / Notes / MCP) on the right, and no overflow menu. Tablets differ only in that the sessions list is a resizable persistent sidebar and the header dropdowns are anchored popovers.
